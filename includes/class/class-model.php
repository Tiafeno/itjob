...
final class itModel {
  // TODO: Vous avez epuiser le nombre limite pour voir les CV des candidates.
  ...
  public function update_interest_status($id_request, $status = 0) {
    global $wpdb;
    if (!is_user_logged_in() || !is_int($id_request)) return false;
    $table = $wpdb->prefix . 'cv_request';
  ...
  public function interest_access($id_candidat, $id_company) {
    global $wpdb;
    if (!is_user_logged_in()) return false;
    if (!$id_company || !$id_candidat) return null;
    $table = $wpdb->prefix . 'cv_request';
