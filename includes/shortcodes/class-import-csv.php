  ...
  class scImport {
    ...
    protected function add_user( $content_type ) {
      ...
      switch ( $content_type ) {
        ...
        case 'user_candidate_informations':
          list( $id, $id_user, $audition, $find_job, $greeting, $firstname, $lastname, $email, $address, $region, $cellphones, $birthday,
          ...
          if ( $candidat_id ) {
            ...
            $listOfExperiences[] = [
              ...
              'old_value'        => [
                ...
                'exp_branch_activity' => isset($abranch_values) && is_array($abranch_values) ? implode(', ', $abranch_values) : '' // Secteur d'activité
              ],
              'validated'       => 1
            ];
    ...
    protected function add_offer() {
      ...
      if ( ! is_wp_error( $post_id ) ) {
        ...
        $acfDateLimit = date('Y-m-d', $obj->date_limit_candidature);
        ...
        if ($Company->branch_activity !== null || !empty($Company->branch_activity)) {
          if (isset($Company->branch_activity->term_id))
